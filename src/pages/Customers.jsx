    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    <h1 className={`text-3xl font-black ${darkMode ? 'text-slate-100' : 'text-slate-800'}`}>Mijozlar</h1>
                    <p className={`text-sm mt-1 font-bold ${sec}`}>
                        Jami {customers.length} ta hamkor
                    </p>
                </div>
                <button
                    onClick={handleAdd}
            ...
            <div className={`p-5 rounded-3xl border ${tb} shadow-sm space-y-4`}>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    >
                        ...
                        <option value="individual">Oddiy Mijoz</option>
            ...
            <div className={`rounded-3xl shadow-sm overflow-hidden border ${tb}`}>
                ...
                <div className="md:hidden divide-y divide-slate-100 dark:divide-slate-700/50">
                    {filteredCustomers.length === 0 ? (
                        <div className="p-10 text-center opacity-50">Mijozlar topilmadi</div>
                        ...
                        <div key={customer.id} className="p-5 space-y-4 active:bg-slate-50 dark:active:bg-slate-700/20 transition-colors">
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                <div className={`p-3 rounded-2xl border ${darkMode ? 'bg-slate-900/40 border-slate-700/50' : 'bg-slate-50 border-slate-100'}`}>
                                    <div className="text-[9px] font-black uppercase tracking-widest text-slate-400 mb-1 flex items-center gap-1.5"><Phone size={12} /> Aloqa</div>
                                    <div className={`text-xs font-black truncate ${darkMode ? 'text-slate-200' : 'text-slate-700'}`}>{customer.phone || '-'}</div>
                                </div>
                                <div className={`p-3 rounded-2xl border ${darkMode ? 'bg-slate-900/40 border-slate-700/50' : 'bg-slate-50 border-slate-100'}`}>
                                    <div className="text-[9px] font-black uppercase tracking-widest text-slate-400 mb-1 flex items-center gap-1.5"><Package size={12} /> Buyurtmalar</div>
                                    <div className={`text-xs font-black ${darkMode ? 'text-blue-400' : 'text-blue-600'}`}>{customer._count?.orders || 0} ta</div>
                                </div>
                            </div>
                    ...
                    <table className="min-w-full text-left">
                        <thead>
                            <tr className={`border-b text-[10px] font-black uppercase tracking-widest ${darkMode ? 'bg-slate-900/60 border-slate-700 text-slate-500' : 'bg-slate-50 border-slate-100 text-slate-500'}`}>
                                <th className="px-6 py-4">Ism & Ma'lumot</th>
